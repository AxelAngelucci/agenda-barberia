...
export async function getBarberia(): Promise<Barberia | null> {
  try {
    ...
    return {
      ...
      servicioSemiPermanenteEnabled:
        data.servicio_semi_permanente_enabled || false,
 ...
 */
export async function updateBarberia(
  data: Partial<Barberia>,
): Promise<boolean> {
  try {
    const supabase = createClient();
    const {
    ...
    if (!user) {
      console.error("No hay usuario autenticado");
      alert(
        "❌ Error: No hay usuario autenticado. Por favor, inicia sesión nuevamente.",
      );
      return false;
    }
    ...
    if (data.precioCejas !== undefined)
      updateData.precio_cejas = data.precioCejas;
    if (data.precioBarba !== undefined)
      updateData.precio_barba = data.precioBarba;
    if (data.precioColor !== undefined)
      updateData.precio_color = data.precioColor;
    if (data.precioAlisado !== undefined)
      updateData.precio_alisado = data.precioAlisado;
    if (data.precioSemiPermanente !== undefined)
      updateData.precio_semi_permanente = data.precioSemiPermanente;
    if (data.servicioColorEnabled !== undefined)
      updateData.servicio_color_enabled = data.servicioColorEnabled;
    if (data.servicioAlisadoEnabled !== undefined)
      updateData.servicio_alisado_enabled = data.servicioAlisadoEnabled;
    if (data.servicioSemiPermanenteEnabled !== undefined)
      updateData.servicio_semi_permanente_enabled =
        data.servicioSemiPermanenteEnabled;
    if (data.datosExtra !== undefined) updateData.datos_extra = data.datosExtra;
    if (data.horariosDisponibles)
      updateData.horarios_disponibles = data.horariosDisponibles;
    ...
    if (error) {
      ...
      if (
        error.message?.includes("precio_cejas") ||
        error.message?.includes("precio_barba")
      ) {
        alert(
          "⚠️ Error: Debes ejecutar el script de migración SQL en Supabase primero.\n\nAbre el archivo supabase/migration_servicios.sql y ejecútalo en el SQL Editor de Supabase.",
        );
      }
